...
if [[ "$USE_NODE_LOCAL" -eq 1 ]]; then
  ...
  export SLURM_CPU_BIND=none
  srun --cpu-bind=none --nodes=2 --ntasks-per-node=1 bash -c '\
    set -euo pipefail; \
  ...
  srun --cpu-bind=none --nodes=2 --ntasks-per-node=8 bash -c '\
    set -euo pipefail; \
    REPO_ROOT_LOCAL="${SLURM_TMPDIR:-/tmp}/lumi-ml-workflows"; \
    python "$REPO_ROOT_LOCAL/workflows/llm_infer/infer.py" \
      --config "$REPO_ROOT_LOCAL/workflows/llm_infer/config.yaml"'
  exit 0
fi
...
srun --cpu-bind=none --nodes=2 --ntasks-per-node=8 \
  python "$REPO_ROOT/workflows/llm_infer/infer.py" \
  --config "$REPO_ROOT/workflows/llm_infer/config.yaml"
